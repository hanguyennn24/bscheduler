...
@app.route('/api/generate_example_data', methods=['GET', 'POST'])
def generate_example_data():
    ...
    if request.method == 'POST':
        data = request.json
        # Get days and shifts from POST data (current time configuration)
        ...
        seed = data.get('seed', 135)
        employee_count = data.get('employee_count', 20)
    else:
        # GET request - use defaults or query parameters
        seed = request.args.get('seed', 135, type=int)
        employee_count = request.args.get('employees', 20, type=int)
        # Use default days and shifts for GET requests
    ...
    # STEP 1: Create employees with GUARANTEED feasible solution
    # Calculate total shifts for better planning
    total_shifts = len(days) * len(shifts)
    avg_shift_hours = sum(s['hours'] for s in shifts) / len(shifts) if shifts else 4
    
    # Make 80% of employees responsible to ensure excellent coverage
    responsible_count = max(5, int(employee_count * 0.8))
    
    for i, name in enumerate(employee_names):
        can_be_responsible = i < responsible_count
        
        # VERY conservative minimum hours to ensure feasibility
        if can_be_responsible:
            # Responsible employees: require only 1-2 shifts worth of hours
            min_hours = max(1, int(avg_shift_hours * random.uniform(1.0, 2.0)))
        else:
            # Regular employees: even more flexible
            min_hours = max(1, int(avg_shift_hours * random.uniform(0.5, 1.5)))
        
        # Maximum hours should be generous
        max_hours = min_hours + int(avg_shift_hours * random.uniform(3.0, 6.0))
        max_hours = min(max_hours, 60)  # More generous cap
        wage = 15
        
        # Generate high availability (90%) for better feasibility
        availability = {}
        for day in days:
            for shift in shifts:
                key = f"{name}_{day}_{shift['name']}"
                # Higher availability for better feasibility
                availability[key] = random.random() < 0.90
        
        # CRITICAL: Ensure each employee can easily meet their minimum hours
        available_hours = 0
        for key, is_avail in availability.items():
            if is_avail:
                parts = key.split('_')
                shift_name = '_'.join(parts[2:])
                shift_hours = next((s['hours'] for s in shifts if s['name'] == shift_name), avg_shift_hours)
                available_hours += shift_hours
        
        # If still not enough, force more availability
        if available_hours < min_hours:
            unavailable_keys = [key for key, is_avail in availability.items() if not is_avail]
            random.shuffle(unavailable_keys)  # Randomize selection
            hours_needed = min_hours - available_hours
    ...
    # STEP 3: Generate ULTRA-conservative staffing requirements for guaranteed feasibility
    ...
    for day in days:
        for shift in shifts:
            ...
            # ULTRA-conservative: NO minimums initially - let the optimizer decide
            min_employees_per_shift[key] = 0
            
            # Generous maximums to give the optimizer flexibility
            max_employees_per_shift[key] = max(1, min(available_employees, 6))
    
    # STEP 4: Final validation - ensure ultra-conservative approach for guaranteed feasibility
    # Since we're using zero minimums, just verify we have reasonable capacity
    total_employee_capacity = sum(emp['max_hours'] for emp in employees)
    total_employee_minimum = sum(emp['min_hours'] for emp in employees)
    
    # Safety check: if total minimum hours exceed reasonable capacity, reduce some minimums
    if total_employee_minimum > total_employee_capacity * 0.8:
        # Sort employees by minimum hours (descending) and reduce the highest ones
        employees_by_min = sorted(employees, key=lambda x: x['min_hours'], reverse=True)
        hours_to_reduce = total_employee_minimum - int(total_employee_capacity * 0.6)
        
        for emp in employees_by_min:
            if hours_to_reduce <= 0:
                break
            reduction = min(hours_to_reduce, emp['min_hours'] // 2)
            emp['min_hours'] = max(1, emp['min_hours'] - reduction)
            hours_to_reduce -= reduction
    ...
    return jsonify({
        ...
        'debug_info': {
            ...
            'total_available_hours': sum(
                sum(next((s['hours'] for s in shifts if s['name'] == '_'.join(k.split('_')[2:])), 4) 
                    for k, v in emp['availability'].items() if v) for emp in employees
            ),
