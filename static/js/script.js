...
document.addEventListener('DOMContentLoaded', function() {
    ...
    async function loadExampleData() {
        try {
            // Get current time configuration from UI
            const currentDays = getSelectedDays();
            const currentShifts = getShiftDetails();
            
            // Validate current configuration
            if (currentDays.length === 0) {
                alert('Please select at least one working day before loading example data.');
                return;
            }
            
            if (currentShifts.length === 0) {
                alert('Please define at least one shift before loading example data.');
                return;
            }
            
            // Send POST request with current time configuration
            ...
            if (data.status === 'success') {
                ...
                // Note: We don't update days and shifts since we're using the current configuration
                // The example data will be generated based on the user's current time settings
                ...
                alert(`Example data loaded successfully! 20 employees with realistic scheduling data have been added using your current time configuration (${currentDays.length} days, ${currentShifts.length} shifts).`);
            }
        } catch (error) {
